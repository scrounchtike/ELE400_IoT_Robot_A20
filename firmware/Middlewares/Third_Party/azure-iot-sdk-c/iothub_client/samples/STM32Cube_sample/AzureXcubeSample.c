...
#include "robiot_cfg.h" 			//AJOUTER PAR OPTIMUSPRIME
...
#define MODEL_DEFAULT_TELEMETRYINTERVAL    25	/* Default is 5 */ ///MODIFIER PAR OPTIMUS PRIME
#define MODEL_TELEMETRYINTERVAL_MULTIPLIER 1	/* Default is 1000 (m) *////MODIFIER PAR OPTIMUS PRIME
...
DECLARE_MODEL(SerializableIotSampleDev_t,
	...
	WITH_DATA(ascii_char_ptr, jobStatus),//AJOUTER PAR OPTIMUSPRIME/////////////////////
	WITH_DATA(ascii_char_ptr, status),//AJOUTER PAR OPTIMUSPRIME/////////////////////////
	WITH_DATA(ascii_char_ptr, jobId),//AJOUTER PAR OPTIMUSPRIME/////////////////////////
	WITH_DATA(ascii_char_ptr, raz),//AJOUTER PAR OPTIMUSPRIME//////////////////////////
	
    /* Methods */
    WITH_METHOD(Reboot),
    WITH_METHOD(Hello, ascii_char_ptr, msg),
    
    //AJOUTER PAR OPTIMUSPRIME//////////////////////////
	WITH_METHOD(Command, ascii_char_ptr, cmd, ascii_char_ptr, jobId, ascii_char_ptr, direction, ascii_char_ptr, value, ascii_char_ptr, speed),
	//AJOUTER PAR OPTIMUSPRIME//////////////////////////
	WITH_METHOD(LoopTelemetry),
...
SerializableIotSampleDev_t * mdl_g = 0;
...
//AJOUTER PAR OPTIMUSPRIME///////////////////////////////////////////////////////////
robiot_mc_t robiot;
robiot_mc_cmd_t robiot_next_cmd;
...
//AJOUTER PAR OPTIMUS PRIME////////////////////////////////////////////////////////////
METHODRETURN_HANDLE Command(SerializableIotSampleDev_t *dev, ascii_char_ptr cmd, ascii_char_ptr jobId, ascii_char_ptr direction, ascii_char_ptr value, ascii_char_ptr speed)
{
  (void)(dev);
  METHODRETURN_HANDLE result = MethodReturn_Create(1, "{\"Message\":\"Command with Method success\"}");
  msg_info("Received Command cmd:%s job_id:%s value:%s speed:%s\n", cmd, jobId, value, speed);

  if ((!strncmp(cmd, "linear", 6))) {
	  robiot.cmd.cmd_type = LINEAR;
  } else if ((!strncmp(cmd, "rotation", 9))) {
	  robiot.cmd.cmd_type = ROTATION;
  } else if ((!strncmp(cmd, "reset", 6))) {
	  robiot.cmd.cmd_type = RESET_ROBOT;
  } else if ((!strncmp(cmd, "stop", 5))){
	  robiot.cmd.cmd_type = STOP;
  }

  mdl_g->jobId = jobId;
  robiot.cmd.job_id = atoi(jobId);
  robiot.cmd.distance = atoi(value);
  robiot.cmd.direction = atoi(direction);
  robiot.cmd.travel_speed = atoi(speed);
  robiot.job_complete = 0;

  return result;
}

METHODRETURN_HANDLE LoopTelemetry(SerializableIotSampleDev_t *dev)
{
  (void)(dev);
  METHODRETURN_HANDLE result = MethodReturn_Create(1, "{\"Message\":\"Telemetry with Method\"}");
  msg_info("Received Telemetry request.\n");
  g_loopTelemetry = true;
  return result;
}
//JUSQUICI////////////////////////////////////////////////////////////
...
/** Main loop */
void cloud_run(robiot_mc_t* robiot_ptr)
{

//AJOUTER/MODIFIER PAR OPTIMUSPRIME//////////////////////////////////////
	robiot = *robiot_ptr;
...
#endif /* CLD_OTA */
  const char * connectionString = AZURE_CONNECTION_STRING;
...
////AJOUTER PAR OPTIMUS PRIME /////////////////////////////////////////////////////////
	robiot_motor_control_init(&robiot);

	robiot_next_cmd.cmd_type = ROTATION;
	robiot_next_cmd.direction = CCLOCKWISE;
	robiot_next_cmd.distance = 25;
	robiot_next_cmd.travel_speed = 80;

	robiot.job_complete = 0;

	robiot.cmd = robiot_next_cmd;
	mdl_g->jobId = "0";
	mdl_g->jobStatus = "Begin";
	mdl_g->status = "Ready";
////Jusqu'ici/////////////////////////////////////////////////////////
...
////AJOUTER PAR OPTIMUS PRIME /////////////////////////////////////////////////////////
      ...
      if (!robiot.job_complete) {
		  robiot_motor_control_process_cmd(robiot, robiot.cmd);
      }
////Jusqu'ici/////////////////////////////////////////////////////////
...
//MODIFIER PAR OPTIMUS PRIME//////////////////////////////////////////////////////////
      int32_t left_ms = comp_left_ms(last_telemetry_time_ms, HAL_GetTick(), device->serModel->TelemetryInterval * MODEL_TELEMETRYINTERVAL_MULTIPLIER);
...
//MODIFIER PAR OPTIMUS PRIME
          if (!mdl_g->jobStatus) {
        	  mdl_g->jobStatus = "Waiting for job";
          }
          ...
          if (!robiot.job_complete) {
			  mdl_g->jobStatus = "Job Running";
			  mdl_g->status = "Not Ready";
          }
////Jusqu'ici/////////////////////////////////////////////////////////
      ...
      ThreadAPI_Sleep(10);//REDUIT PAR OPTIMUSPRIME////////////////////////////////////////////////////
...
//////////////////AJOUTER PAR OPTIMUS PRIME Jusqu'a la fin/////////////////////////////////////////////
/**
...
static void MX_TIM5_Init(void)
{
  ...
  htim5.Instance = TIM5;
  htim5.Init.Prescaler = 645;//////////////////////MODIFIER PAR OPTIMUS_PRIME//////////////////
...
{
	encoder_count++;
	if (encoder_count >= robiot.cmd.distance) {
		robiot_motor_control_set_pwm(robiot.htim, TIM_CHANNEL, ROBIOT_PWM_PERIOD, 0);
		encoder_count = 0;
        mdl_g->jobStatus = "Job Complete";
        mdl_g->status = "Ready";
        robiot.job_complete = 1;
	}
}

int robiot_get_raz(void)
{
	int raz_flag = 0;
