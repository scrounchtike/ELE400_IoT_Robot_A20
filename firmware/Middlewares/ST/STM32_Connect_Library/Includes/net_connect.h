...
typedef enum enum_state
{
  ...
  NET_STATE_STARTED, /**< Network interface interface is started, MAC address can be retrieved */
  ...
  NET_STATE_STOPPED,  /**< Network interface interface is stopped, IP address is zeroed , socket operation can not be connected anymore */

  NET_STATE_DISCONNECTING, /**< Network interface interface is disconnecting */
  NET_STATE_DISCONNECTED,  /**< Network interface interface is disconnected  */
  NET_STATE_LINK_LOST,     /**< Network interface connection is lost , this can be a transient state , it can return to connected state without application specific action*/
...
int32_t net_if_ping(net_if_handle_t *pnetif, sockaddr_t *addr, int32_t count, int32_t delay, int32_t reponse[]);
