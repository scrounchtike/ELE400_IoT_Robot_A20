...
CS_Status_t CS_power_off(void)
{
  ...
  /* 1st step: Software Power OFF of Cellular module */
  if (DATAPACK_writeStruct(&cmd_buf[0],
                           (uint16_t) CSMT_NONE,
                           (uint16_t) 0U,
                           NULL) == DATAPACK_OK)
  ...
  /* 2nd step: Hardware Power OFF of Cellular module */
...
CS_Status_t CS_sim_select(CS_SimSlot_t simSelected)
{
                           ...
                           (void *)&simSelected) == DATAPACK_OK)
  {
    ...
    if (err == ATSTATUS_OK)
    {
      PrintDBG("<Cellular_Service> SIM %d selected", simSelected)
      retval = CELLULAR_OK;
    }
  }
  ...
  if (retval == CELLULAR_ERROR)
  {
    PrintErr("<Cellular_Service> error with sim selection")
  }
  return (retval);
}
...
CS_Status_t CS_init_modem(CS_ModemInit_t init, CS_Bool_t reset, const CS_CHAR_t *pin_code)
{
  ...
  (void) memcpy((void *)&modemInit_struct.pincode.pincode[0],
                ...
                strlen((const cs_TYPE_CHAR_t *)pin_code));
...
CS_Status_t CS_define_pdn(CS_PDN_conf_id_t cid, const CS_CHAR_t *apn, CS_PDN_configuration_t *pdn_conf)
{
  ...
  (void) memcpy((void *)&pdn_infos.apn[0],
                (const CS_CHAR_t *)apn,
                strlen((const cs_TYPE_CHAR_t *)apn));
                              ...
                              &ip_addr_info) == DATAPACK_OK)
      {
        ...
        *ip_addr_type = ip_addr_info.ip_addr_type;
        (void) memcpy((void *)p_ip_addr_value,
                      ...
                      strlen((cs_TYPE_CHAR_t *)ip_addr_info.ip_addr_value));
                                  ...
                                  CS_PDN_conf_id_t cid)
{
  ...
  PrintINFO("allocated socket handle=%ld (local)", sockhandle)
  return (sockhandle);
}
                            ...
                            uint16_t local_port)
{
  ...
  {
    PrintErr("<Cellular_Service> socket bind allowed only after create/before connect %ld ", sockHandle)
    return (CELLULAR_ERROR);
  }
                                     ...
                                     cellular_socket_closed_callback_t remote_close_cb)
{
  ...
  {
    PrintErr("<Cellular_Service> invalid socket handle %ld (set cb)", sockHandle)
    return (CELLULAR_ERROR);
  }
                            ...
                            uint32_t length)
{
  ...
  {
    PrintErr("<Cellular_Service> socket not connected (state=%d) for handle %ld (send)",
                              ...
                              uint16_t remote_port)
{
  ...
  (void) memcpy((void *)send_data_struct.ip_addr_value,
                ...
                strlen((cs_TYPE_CHAR_t *)p_ip_addr_value));
                           ...
                           (void *)&send_data_struct) == DATAPACK_OK)
  {
    ...
    if (err == ATSTATUS_OK)
    {
      PrintDBG("<Cellular_Service> socket data sent (sendto)")
      retval = CELLULAR_OK;
    }
  }
  ...
  if (retval == CELLULAR_ERROR)
  {
    PrintErr("<Cellular_Service> error when sending data to socket (sendto)")
  }
  return (retval);
}
                           ...
                           uint32_t max_buf_length)
{
  ...
  if (cs_ctxt_sockets_info[sockHandle].state != SOCKETSTATE_CONNECTED)
  {
    PrintErr("<Cellular_Service> socket not connected (state=%d) for handle %ld (rcv)",
             cs_ctxt_sockets_info[sockHandle].state,
             sockHandle)
    return (-1);
  }
                              ...
                              &rx_data_from) == DATAPACK_OK)
      {
        ...
        *p_ip_addr_type = rx_data_from.ip_addr_type;
        (void) memcpy((void *)p_ip_addr_value,
                      ...
                      strlen((cs_TYPE_CHAR_t *)rx_data_from.ip_addr_value));
...
CS_Status_t CDS_socket_close(socket_handle_t sockHandle, uint8_t force)
{
  ...
  switch (cs_ctxt_sockets_info[sockHandle].state)
  {
    ...
    case SOCKETSTATE_NOT_ALLOC:
      PrintErr("<Cellular_Service> invalid socket handle %ld (close)", sockHandle)
      retval = CELLULAR_ERROR;
      break;
                                  ...
                                  CS_SocketCnxInfos_t *p_infos)
{
  ...
  if (cs_ctxt_sockets_info[sockHandle].state != SOCKETSTATE_CONNECTED)
  {
    PrintErr("<Cellular_Service> socket not connected (state=%d) for handle %ld (status)",
             cs_ctxt_sockets_info[sockHandle].state,
             sockHandle)
    return (CELLULAR_ERROR);
  }
                     ...
                     cellular_ping_response_callback_t cs_ping_rsp_cb)
{
                           ...
                           (void *)&loc_ping_params) == DATAPACK_OK)
  {
    ...
    if (err == ATSTATUS_OK)
    {
      ...
      {
        PrintINFO("<Cellular_Service> Ping transaction finished")
      ...
      {
        PrintINFO("<Cellular_Service> Waiting for ping reports")
      }
      retval = CELLULAR_OK;
    }
  }
...
CS_Status_t CS_direct_cmd(CS_direct_cmd_tx_t *direct_cmd_tx, cellular_direct_cmd_callback_t direct_cmd_callback)
{
  ...
  if (direct_cmd_tx->cmd_size <= MAX_DIREXT_CMD_SIZE)
  {
                          ...
                          (void *)direct_cmd_tx) == DATAPACK_OK)
    {
      ...
      if (err == ATSTATUS_OK)
      {
        PrintDBG("<Cellular_Service> Direct command infos received")
        retval = CELLULAR_OK;
      }
      else
      {
  ...
  {
    PrintINFO("<Cellular_Service> Direct command command size to big (limit=%d)", MAX_DIREXT_CMD_SIZE)
    retval = CELLULAR_ERROR;
  }

  if (retval == CELLULAR_ERROR)
  {
    PrintErr("<Cellular_Service> error when sending direct cmd")
  }
  return (retval);
}
...
static CS_Status_t perform_HW_reset(void)
{
  ...
  if (retval == CELLULAR_ERROR)
  {
    PrintDBG("<Cellular_Service> error during HW device reset")
  }
  return (retval);
}
...
static CS_Status_t perform_SW_reset(void)
{
  ...
  if (retval == CELLULAR_ERROR)
  {
    PrintDBG("<Cellular_Service> error during SW device reset")
  }
  return (retval);
}
...
static void CELLULAR_urc_notif(at_buf_t *p_rsp_buf)
{
  ...
  else if (msgtype == (uint16_t) CSMT_URC_PING_RSP)
  {
    ...
    if (DATAPACK_readStruct(p_rsp_buf,
                            ...
                            (void *)&ping_rsp) == DATAPACK_OK)
    {
      PrintINFO("ping URC received at CS level")
      if (urc_ping_rsp_callback != NULL)
      {
        (* urc_ping_rsp_callback)(ping_rsp);
      }
...
static void socket_init(socket_handle_t index)
{
  PrintAPI("<Cellular_Service> SOCKET_init (index=%ld)", index)
...
static void socket_deallocateHandle(socket_handle_t sockhandle)
{
  PrintINFO("socket_deallocateHandle %ld", sockhandle)
                                 ...
                                 CS_PDN_conf_id_t cid)
{
  ...
  {
    PrintErr("<Cellular_Service> socket handle %ld not available", sockhandle)
    retval = CELLULAR_ERROR;
  }
                               ...
                               uint16_t local_port)
{
  ...
  if (cs_ctxt_sockets_info[sockhandle].state == SOCKETSTATE_NOT_ALLOC)
  {
    PrintErr("<Cellular_Service> invalid socket handle %ld (bind)", sockhandle)
    retval = CELLULAR_ERROR;
  }
  else
  {
                                    ...
                                    void *p_opt_val)
{
  ...
  if (cs_ctxt_sockets_info[sockhandle].state == SOCKETSTATE_NOT_ALLOC)
  {
    PrintErr("<Cellular_Service> invalid socket handle %ld (cfg)", sockhandle)
    retval = CELLULAR_ERROR;
  }
    ...
    else if (opt_level == CS_SOL_TRANSPORT)
    {
      switch (opt_name)
      {
        ...
        case CS_SON_TRP_CONNECT_SETUP_TIMEOUT:
        {
          ...
          {
            PrintErr("<Cellular_Service> parameter value out of range ")
            retval = CELLULAR_ERROR;
          }
          break;
        }
                                           ...
                                           uint16_t remote_port)
{
  ...
  if (cs_ctxt_sockets_info[sockhandle].state == SOCKETSTATE_NOT_ALLOC)
  {
    PrintErr("<Cellular_Service> invalid socket handle %ld (cfg remote)", sockhandle)
    return (CELLULAR_ERROR);
  }
  ...
  (void) memcpy((void *) &cs_ctxt_sockets_info[sockhandle].ip_addr_value, (void *)p_ip_addr_value,
                strlen((const cs_TYPE_CHAR_t *)p_ip_addr_value));
