                                   ...
                                   uint8_t reserved_user_modem_cid)
{
  ...
  PrintErr("unexpected error reserve_user_modem_cid()")
  return;
}
...
CS_IPaddrType_t atcm_get_ip_address_type(AT_CHAR_t *p_addr_str)
{
  ...
  */
  ...
  uint8_t str_size_cpt = (uint8_t) strlen((const atcm_TYPE_CHAR_t *)p_addr_str);
  ...
  do
  {
    ...
    pTmp++;
    str_size_cpt--;
  }  while ((pTmp != NULL) && (str_size_cpt > 0U));
...
at_status_t atcm_select_hw_simslot(CS_SimSlot_t sim)
{
  ...
  */
  ...
  switch (sim)
  {
    ...
    default:
      converted_sim = SC_MODEM_SIM_SOCKET_0;
      PrintErr("Invalid sim slot value, use default one")
      break;
  }
                                 ...
                                 uint32_t *p_ATcmdTimeout)
{
  ...
  uint8_t string_length = (uint8_t) strlen((const atcm_TYPE_CHAR_t *) p_cmd_name_string);
  (void) memcpy((atcm_TYPE_CHAR_t *)p_atp_ctxt->current_atcmd.name,
                p_cmd_name_string,
                string_length);

  PrintDBG("<modem custom> build the cmd %s (type=%d, length=%d)",
  ...
  switch (p_atp_ctxt->current_SID)
  {
    ...
    case SID_CS_RECEIVE_DATA_FROM:
    {
      ...
      (void) memcpy((void *)&rxdata_from.ip_addr_value,
                    ...
                    strlen((atcm_TYPE_CHAR_t *)p_modem_ctxt->socket_ctxt.socketReceivedata.ip_addr_value));
      ...
      {
        PrintErr("Receive data from problem")
        retval = ATSTATUS_ERROR;
      }
      break;
    }
...
void atcm_reset_persistent_context(atcustom_persistent_context_t *p_persistent_ctxt)
{
  ...
  for (uint8_t i = 0U; i < CELLULAR_MAX_SOCKETS; i++)
  {
    ...
    p_tmp->socket_connId_value = ((uint8_t)i + 1U); /* socket ID range from 1 to 6, if a modem does not support this range => need to adapt */
...
void atcm_reset_SID_context(atcustom_SID_context_t *p_sid_ctxt)
{
  ...
  (void) memset((void *)&p_sid_ctxt->read_operator_infos, 0, sizeof(CS_OperatorSelector_t));
  ...
  (void) memset((void *)&p_sid_ctxt->error_report, 0, sizeof(csint_error_report_t));
                                ...
                                at_element_info_t *element_infos)
{
  ...
  {
    /* if string length > 0 */
    if (strlen((const atcm_TYPE_CHAR_t *)(p_modem_ctxt->p_modem_LUT)[i].cmd_str) > 0U)
    {
      /* compare strings size first */
      if ((strlen((const atcm_TYPE_CHAR_t *)(p_modem_ctxt->p_modem_LUT)[i].cmd_str) == element_infos->str_size))
      {
...
at_bool_t atcm_get_modem_data_mode(const atcustom_modem_context_t *p_modem_ctxt)
{
