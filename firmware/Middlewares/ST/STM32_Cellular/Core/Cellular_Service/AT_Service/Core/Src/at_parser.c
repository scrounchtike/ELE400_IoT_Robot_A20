...
static bool write_data2buffer(uint8_t *p_ATcmdBuf, const AT_CHAR_t *p_str, uint16_t str_size, uint16_t *cmd_total_length, uint16_t *remaining_size);
...
void ATParser_init(at_context_t *p_at_ctxt, IPC_CheckEndOfMsgCallbackTypeDef *p_checkEndOfMsgCallback)
{
  ...
  *p_checkEndOfMsgCallback = atcc_checkEndOfMsgCallback(p_at_ctxt);
  ...
  (void) sprintf((atp_TYPE_CHAR_t *)p_at_ctxt->parser.endstr, "\r");
...
/**
  ...
  */
static bool write_data2buffer(uint8_t *p_ATcmdBuf, const AT_CHAR_t *p_str, uint16_t str_size, uint16_t *p_cmd_total_length, uint16_t *p_remaining_size)
{
...
static uint16_t build_command(at_context_t *p_at_ctxt, uint8_t *p_ATcmdBuf, uint16_t ATcmdBuf_maxSize)
{
  /*
    ...
    *  <cmd_sep> is the separator between name and params for this type of command (exple: for a write command we have "=")
  ...
  */
  ...
  switch (cmd_type)
  {
    case ATTYPE_TEST_CMD:
    case ATTYPE_READ_CMD:
    case ATTYPE_WRITE_CMD:
    case ATTYPE_EXECUTION_CMD:
    {
      ...
      str_size = (uint16_t) strlen((atp_TYPE_CHAR_t *) &p_at_ctxt->parser.current_atcmd.name);
      (void) write_data2buffer(p_ATcmdBuf, p_str, str_size, &cmd_total_length, &remaining_size);
      ...
      str_size = (uint16_t) strlen((atp_TYPE_CHAR_t *) &p_at_ctxt->parser.current_atcmd.params);
      (void) write_data2buffer(p_ATcmdBuf, p_str, str_size, &cmd_total_length, &remaining_size);

      /* build <cmd_endstr> part */
      p_str = p_at_ctxt->parser.endstr;
      str_size = (uint16_t) strlen((atp_TYPE_CHAR_t *) &p_at_ctxt->parser.endstr);
      (void) write_data2buffer(p_ATcmdBuf, p_str, str_size, &cmd_total_length, &remaining_size);
      break;
    }
    ...
    case ATTYPE_UNKNOWN_CMD:
    default:
    {
      cmd_total_length = 0U;
      PrintErr("invalid command type")
      break;
    }
  }
