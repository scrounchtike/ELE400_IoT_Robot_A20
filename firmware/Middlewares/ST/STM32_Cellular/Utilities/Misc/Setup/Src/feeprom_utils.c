...
static uint32_t feeprom_utils_find_bank(setup_appli_code_t appli_code, setup_appli_version_t appli_version, setup_config_t **addr,
                                        ...
                                        uint8_t *config_addr, uint32_t config_size)
{
  ...
  if (res != FEEPROM_UTILS_NO_BANK_FOUND)
  {
    ...
    if (res == 0U)
    {
      ...
      {
        count = byteswritten;
        res = feeprom_utils_flash_write((uint8_t*)&config_header, (uint8_t*)feeprom_config, sizeof(setup_config_header_t), &byteswritten);
        if (res != 0U)
        {
          count = 0U;
        }
  ...
  {
    PrintSetup("Save config fail: no available bank found")
  }
  return count;
}
                                        ...
                                        uint8_t **config_addr, uint32_t *config_size)
{
  ...
  ret = feeprom_utils_find_bank(appli_code, appli_version, &setup_config, &page_number);

  if (ret == FEEPROM_UTILS_MATCHING_BANK_FOUND)
  {
