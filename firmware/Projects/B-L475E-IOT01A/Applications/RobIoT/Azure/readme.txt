...
The application supports the B-L475E-IOT01 board and connects to the Internet through the on-board or attached
network interface.
...
@par Hardware and Software environment
  ...
  - MCU board: B-L475E-IOT01 (MB1297 rev D), with FW "Inventek eS-WiFi ISM43362-M3G-L44-SPI C3.5.2.3.BETA9"
      Note: the WiFi FW version is displayed on the board console at boot time.
...
STMICROELECTRONICS web dashboard for AZURE

An AZURE-based web dashboard is available to for an easy sensor data visualization and device control.

On the device side:
- Open your IDE preprocessor defined symbols tab and add AZURE_USE_STM_DASHBOARD.
- Build
- Start the board as indicated in the previous sections
- With the board console, enter the AZURE credentials as follows:
   * Enter the Wi-Fi credentials if not yet done
   * Enter the Root CA if not yet done. See above section "Directory contents"
   * Update your IoT device connection parameters with: DpsEndpoint=global.azure-devices-provisioning.net;IdScope=0ne00081380;

On the web application side:
  - Open it at: https://stm32ode-v2.azurewebsites.net
  - Create an account for the 1st time
  - Create a device using the 12 digit device ID prompted on the device console.
   * The message looks like this: === DeviceID: <12 digits ID> ======

- If everything went well these messages will be displayed to the board console:
    Alias Certificate created.
    iotHubClientHandle Created
    Callbacks registered successfully.
    DeviceTwinCallback payload will be printed (these are the data stored on the cloud side)

You can now use the dashboard:
  - Visualize the reported sensor values
