...
ProtectionTest_t aProtectTests[] =
{
  ...
  {SB_REGION_ROM_END - SHIFT_FLASH_WRITE, "WRP end address", TEST_WRITE_FLASH},          /* -7 : alignment for 64bits writing test */
  ...
  {SE_KEY_REGION_ROM_END - SHIFT_FLASH_WRITE, "PCROP end address", TEST_WRITE_FLASH},    /* -7 : alignment for 64bits writing test */
  ...
  {SE_CODE_REGION_ROM_END - SHIFT_FLASH_WRITE, "Isolated code end address", TEST_WRITE_FLASH},  /* -7 : alignment for 64bits writing test + WRP protection applies */
  ...
  {SE_STARTUP_REGION_ROM_START + 1U, "Isolated startup address", TEST_EXECUTE},          /* +1 : contains the SE startup code that initializes all the variables in the binary. */
  ...
  {REGION_SLOT_0_START + SFU_IMG_IMAGE_OFFSET - (SHIFT_FLASH_WRITE + 1U), "Isolated NVdata address", TEST_WRITE_FLASH}, /* -8 : previous 64 bits for writing test */
  {REGION_SLOT_0_START + SFU_IMG_IMAGE_OFFSET - 1U, "Isolated NVdata address", TEST_READ_FLASH},  /* -1 : previous 8bits for reading test */
  {REGION_SLOT_0_START + SFU_IMG_IMAGE_OFFSET - 4U, "Isolated NVdata address", TEST_EXECUTE},     /* -4 : previous 32bits for execute test */
  ...
  {(uint32_t)0x1FFF7838, "OBs @ 0x1FFF7838", TEST_WRITE_FLASH}, /* Due to MPU definition, we protect 64B instead of 40B */
#endif /* SFU_MPU_PROTECT_ENABLE  */
...
static void SFU_TEST_Protection(void)
{
  ...
  /*
    ...
    * WatchDog should be disabled : while(1) loop in case HardFault when trying to execute code @ address protected by FWALL/Code Isolation
  ...
  do
  {
    ...
    TRACE("\r\n= [TEST] %s @ %s %08x", aTestOperation[aProtectTests[test_idx].type], aProtectTests[test_idx].msg, aProtectTests[test_idx].address + offset);
...
void SFU_TEST_Init(void)
{
  ...
  else if (MAGIC_TEST_INIT != HAL_RTCEx_BKUPRead(&RtcHandle, RTC_BKP_DR0)) /* Test not done nor in progress: Init magic (DR0) and reset test index (DR1) */
  {
