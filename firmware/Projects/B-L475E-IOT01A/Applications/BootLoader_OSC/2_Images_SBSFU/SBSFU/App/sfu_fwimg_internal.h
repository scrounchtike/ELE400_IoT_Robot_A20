...
/**
  ...
  * When SFU_FWIMG_BLOCK_ON_ABNORMAL_ERRORS_MODE is deactivated, a log is printed in the console (if SFU_DEBUG_MODE is activated) and the execution continues.
  */
#if defined(SFU_FWIMG_BLOCK_ON_ABNORMAL_ERRORS_MODE)
#define StatusFWIMG(B,A) if (B) { \
  SFU_IMG_Status=A; \
  SFU_IMG_Line = __LINE__; \
  TRACE("\r\n          Abnormal error %d at line %d in %s - BLOCK", SFU_IMG_Status, SFU_IMG_Line, __FILE__); \
  while(1); \
  } while(0);
#else
#define StatusFWIMG(B,A) if (B) { \
  SFU_IMG_Status=A; \
  SFU_IMG_Line = __LINE__; \
  TRACE("\r\n          Abnormal error %d at line %d in %s - CONTINUE", SFU_IMG_Status, SFU_IMG_Line, __FILE__); \
  } while(0);
...
/**
  ...
  * But, a Firmware can be split in up to 2 areas (typically when a FW installation procedure is interrupted or after a decrypt operation).
  */
typedef struct
{
...
/**
 * Variable describing how the Firmware to restore is written in FLASH.
 * 2 situations can occur:
 *    A FW update has been interrupted while the swap of the images was in progress:
 *       It is made of 2 contiguous areas.
 *       These areas will be used to restore the Firmware as active Firmware.
 *    The previous Firmware has been completely copied in slot #1:
 *       It is made of 1 signle area: slot #1.
 */
SE_Ex_PayloadDescTypeDef fw_desc_to_recover;
/**
...
extern SE_Ex_PayloadDescTypeDef fw_desc_to_recover;
#endif
...
/** @defgroup SFU_IMG_Functions_COMMON_LowLevel Images Handling Low Level Services
  ...
  * @{
  */
/* Check the trailer validity and fill the structure describing how to restore the previous FW */
