/**
  ...
  *          The same services are offered to the user application thanks to a similar file integrated in the user application.
...
static SFU_ErrorStatus WriteInstallHeader(uint8_t *pfw_header)
{
  ...
  if (ret == SFU_SUCCESS)
  {
    ret = SFU_LL_FLASH_Write(&flash_if_info, (void *)(SFU_IMG_SWAP_REGION_BEGIN_VALUE + SE_FW_HEADER_TOT_LEN), (void *)zero_buffer, sizeof(zero_buffer));
  }
  return ret;
}
...
/**
  ...
  *         This function is used by the local loader to know where to store a new Firmware Image before asking for its installation.
