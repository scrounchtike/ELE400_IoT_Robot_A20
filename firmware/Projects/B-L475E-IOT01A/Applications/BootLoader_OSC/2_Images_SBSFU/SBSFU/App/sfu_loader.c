...
#include "sfu_fwimg_services.h"      /* for version checking & to check if a valid FW is installed (the local bootloader is a kind of "application" running in SB_SFU) */
#include "app_sfu.h"
 ...
 */
static uint8_t fw_header[SE_FW_HEADER_TOT_LEN];
static uint32_t m_uDwlAreaAddress = 0U;                          /*!< Adress of to write in download area */
static uint32_t m_uDwlAreaStart = 0U;                            /*!< Adress of download area */
...
SFU_ErrorStatus SFU_LOADER_Init(void)
{
  ...
  */
  ...
  if (0 != (uint32_t)(SFU_COM_YMODEM_PACKET_1K_SIZE % (uint32_t)sizeof(SFU_LL_FLASH_write_t)))
  {
    ...
    TRACE("\r\n= [FWIMG] Packet Payload size (%d) is not matching the FLASH constraints", SFU_COM_YMODEM_PACKET_1K_SIZE);
    return SFU_ERROR;
...
/**
  * @brief Check if a local download is requested.
  * @note  Change this code to implement another local download trigger.
  * @param  None
  * @retval 0 if no local download is requested, 1 if a local download is requested
  *
  */
uint32_t SFU_LOADER_CheckLocalDownloadReq(void)
{
...
/**
  ...
  * @retval SFU_ErrorStatus SFU_SUCCESS if successful, SFU_ERROR otherwise.
  */
SFU_ErrorStatus SFU_LOADER_DownloadNewUserFw(SFU_LOADER_StatusTypeDef *peSFU_LOADER_Status, SFU_FwImageFlashTypeDef *p_FwImageFlashData, uint32_t *puSize)
{
...
/**
  ...
  * @note Please note that when the new image is installed, this metadata is checked by @ref SFU_IMG_CheckCandidateMetadata.
  * @param  peSFU_LOADER_Status: SFU LOADER Status.
  *         This parameter can be a value of @ref SFU_LOADER_Status_Structure_definition.
...
static SFU_ErrorStatus SFU_LOADER_VerifyFwHeader(SFU_LOADER_StatusTypeDef *peSFU_LOADER_Status, uint8_t *pBuffer)
{
  ...
  *peSFU_LOADER_Status = SFU_LOADER_ERR_CMD_AUTH_FAILED;
  ...
  if (SE_VerifyFwRawHeaderTag(&e_se_status, p_x_fw_raw_header) != SE_ERROR)
  {
    ...
    /* if no valid firmware is installed then checking that the new version is greater than or equal to the current version is meaningless */
    if (((((int32_t)p_x_fw_raw_header->FwVersion) >= curVer) || (SFU_ERROR == hasActiveFw)) &&   /* candidate >= current version if any */
        (p_x_fw_raw_header->FwVersion >= SFU_FW_VERSION_START_NUM))                                              /* && candidate >= min.version number  */
    {
      /* The installation is authorized */
#if defined(SFU_VERBOSE_DEBUG_MODE)
      if (SFU_SUCCESS == hasActiveFw)
      {
        /* FW with lower version installed */
        TRACE("\r\n          Anti-rollback: candidate version(%d) accepted | current version(%d) , min.version(%d) !", p_x_fw_raw_header->FwVersion, curVer, SFU_FW_VERSION_START_NUM);
      }
      else
      {
        /* There is no FW at all or a bricked FW in slot #0 so its version is Not Applicable (N.A.) */
        TRACE("\r\n          Anti-rollback: candidate version(%d) accepted | current version(N.A.) , min.version(%d) !", p_x_fw_raw_header->FwVersion, SFU_FW_VERSION_START_NUM);
      }
    ...
    else
    {
      ...
      if (SFU_SUCCESS == hasActiveFw)
      {
        /* FW with upper version installed */
        TRACE("\r\n          Anti-rollback: candidate version(%d) rejected | current version(%d) , min.version(%d) !", p_x_fw_raw_header->FwVersion, curVer, SFU_FW_VERSION_START_NUM);
      }
      else
      {
        /* There is no FW at all or a bricked FW in slot #0 so its version is Not Applicable (N.A.) */
        TRACE("\r\n          Anti-rollback: candidate version(%d) rejected | current version(N.A.) , min.version(%d) !", p_x_fw_raw_header->FwVersion, SFU_FW_VERSION_START_NUM);
      }
#else
      TRACE("\r\n          Anti-rollback: candidate version(%d) rejected | current version(%d) , min.version(%d) !", p_x_fw_raw_header->FwVersion, curVer, SFU_FW_VERSION_START_NUM);
#endif /* SFU_VERBOSE_DEBUG_MODE */
    ...
    if (SFU_SUCCESS == e_ret_status)
    {
      /*
       * ##2 - Check length :
       *      We do not check the length versus the trailer constraint because this check is already implemented in SFU_IMG_FirmwareToInstall().
       *      If the firmware is too big to have some room left for the trailer info then the installation will not be triggered.
       *      The interest is to avoid duplicating the checks (checking just in time).
       *      This is also because in the case of a download handled by the UserApp we cannot rely on the checks done by the UserApp before installing a FW.
       ...
       */
      if (p_x_fw_raw_header->FwSize > (SFU_IMG_SLOT_DWL_REGION_SIZE - SFU_IMG_IMAGE_OFFSET))
      {
      ...
      {
        /* The firmware can be written in slot #1 (but this does not mean it can be installed, this will be checked at the SFU_IMG_FirmwareToInstall() stage.) */
        e_ret_status = SFU_SUCCESS;
...
SFU_ErrorStatus SFU_COM_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  /* Last packet : size fo data to write could be different than SFU_COM_YMODEM_PACKET_1K_SIZE */
  if (m_uPacketsReceived == m_uNbrBlocksYmodem)
  {
    ...
    else
    {
      ...
      uSize = m_uFileSizeYmodem - ((uint32_t)(m_uFileSizeYmodem / SFU_COM_YMODEM_PACKET_1K_SIZE) * SFU_COM_YMODEM_PACKET_1K_SIZE);
    }

    m_uPacketsReceived = 0U;
  }

  /* First packet : Contains the FW header (IMG_OFFSET bytes length) which is not encrypted  */
  if (m_uPacketsReceived == 1)
  {
    ...
    if ((e_ret_status == SFU_SUCCESS) && (SFU_LL_FLASH_Erase_Size(&x_flash_info, (void *) m_uDwlAreaAddress, m_uDwlAreaSize) != SFU_SUCCESS))
    {
      e_ret_status = SFU_ERROR;
    }
  ...
  {
    /* By construction, the length of the buffer (fw_decrypted_chunk or pData) must be a multiple of sizeof(SFU_IMG_write_t) to avoid reading out of the buffer */
