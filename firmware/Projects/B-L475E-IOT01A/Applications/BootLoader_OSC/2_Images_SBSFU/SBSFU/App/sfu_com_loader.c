...
/** @defgroup  SFU_COM_LOADER  SFU Com Local Loader
  * @brief This file provides a set of  functions to manage the Comms interface used for download (local loader with Ymodem protocol over UART).
  * @{
  */
...
/** @defgroup SFU_COM_LOADER_Private_Variables Private Variables
  * @{
  */
static uint8_t m_aPacketData[SFU_COM_YMODEM_PACKET_1K_SIZE + SFU_COM_YMODEM_PACKET_DATA_INDEX + SFU_COM_YMODEM_PACKET_TRAILER_SIZE] __attribute__((aligned(8)));   /*!<Array used to store Packet Data*/
uint8_t m_aFileName[SFU_COM_YMODEM_FILE_NAME_LENGTH + 1U]; /*!< Array used to store File Name data */
...
{
  uint32_t i, packet_length = 0U, session_done = 0U, file_done, errors = 0U, session_begin = 0U;
  uint32_t ramsource = 0U, filesize = 0U,  packets_received = 0;
  uint8_t *file_ptr;
      ...
      switch (ReceivePacket(m_aPacketData, &packet_length, SFU_COM_YMODEM_DOWNLOAD_TIMEOUT))
      {
            ...
            default:
              ...
              else
              {
                ...
                else /* Data packet */
                {
                  ...
                  if (SFU_COM_YMODEM_DataPktRxCpltCallback((uint8_t *) ramsource, (uint32_t) packet_length) == SFU_SUCCESS)
                  {
                    SFU_COM_Serial_PutByte(SFU_COM_YMODEM_ACK);
        ...
        default:
          ...
          else
          {
            ...
            /* Toggling the LED in case no console is available: the toggling frequency depends on SFU_COM_YMODEM_DOWNLOAD_TIMEOUT */
  ...
  if (eRetStatus == SFU_SUCCESS)
  {
    ...
    *pData = char1;
    ...
    if (packet_size >= SFU_COM_YMODEM_PACKET_SIZE)
    {
      ...
      eRetStatus = SFU_LL_UART_Receive(&pData[SFU_COM_YMODEM_PACKET_NUMBER_INDEX], packet_size + SFU_COM_YMODEM_PACKET_OVERHEAD_SIZE, uTimeout);
      ...
      if (eRetStatus == SFU_SUCCESS)
      {
        ...
        if (pData[SFU_COM_YMODEM_PACKET_NUMBER_INDEX] != ((pData[SFU_COM_YMODEM_PACKET_CNUMBER_INDEX]) ^ SFU_COM_YMODEM_NEGATIVE_BYTE))
        {
          packet_size = 0U;
          status = HAL_ERROR;
        }
