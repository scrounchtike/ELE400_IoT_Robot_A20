...
/** @defgroup SFU_CONFIG_WRP_AREAS WRP Protected Areas Configuration
  * @{
  */
#define SFU_PROTECT_WRP_AREA_1          (OB_WRPAREA_BANK1_AREAA)                     /*!< Bank 1, Area A  used to protect Vector Table*/
#define SFU_PROTECT_WRP_PAGE_START_1    ((uint32_t)((SFU_BOOT_BASE_ADDR - FLASH_BASE) / FLASH_PAGE_SIZE)) /*!< First page including the Vector Table: 0 based */
#define SFU_PROTECT_WRP_PAGE_END_1      ((uint32_t)((SFU_AREA_ADDR_END - FLASH_BASE) / FLASH_PAGE_SIZE)) /*!< Last page: (code_size-1)/page_size because the page indexes start from 0 */
/**
  * @}
  */
...
/** @defgroup SFU_CONFIG_FIREWALL_AREAS FIREWALL Configuration
  * @{
  */
#define SFU_PROTECT_FWALL_CODE_ADDR_START   ((uint32_t) SFU_SENG_AREA_ADDR_START)   /*!< Firewall protection CODE area START address*/
#define SFU_PROTECT_FWALL_CODE_SIZE         ((uint32_t) SFU_SENG_AREA_SIZE)         /*!< Firewall protection CODE area SIZE*/
#define SFU_PROTECT_FWALL_NVDATA_ADDR_START ((uint32_t)(FLASH_BASE+(FLASH_BANK_SIZE)))/*!< Firewall protection NVDATA area START address*/
#define SFU_PROTECT_FWALL_NVDATA_SIZE       (SFU_IMG_SLOT_0_REGION_BEGIN_VALUE + SFU_IMG_IMAGE_OFFSET - SFU_PROTECT_FWALL_NVDATA_ADDR_START)/*!< Firewall protection NVDATA area SIZE*/
#define SFU_PROTECT_FWALL_VDATA_ADDR_START  ((uint32_t) SFU_SENG_RAM_ADDR_START)    /*!< Firewall protection VDATA area START address*/
#define SFU_PROTECT_FWALL_VDATA_SIZE        ((uint32_t) SFU_SENG_RAM_SIZE)          /*!< Firewall protection VDATA area SIZE*/
/**
  * @}
  */
...
/**
  * @brief Region 0 - The Aliases Region and all User Flash & internal RAM. When executing inside SB/SFU by default all the Aliases and all the
  *    User Flash (used for UserApp1 and UserApp2) area must not be executable, but used only to read and write the downloaded code
  *    RAM is also not executable.
...
#define SFU_PROTECT_MPU_AREA_USER_SREG          0x00U                                    /*!< Subregion mask to deactivate unexpected MPU subregions */

/**
  * @brief Region 1 and Region 2 - Enable the execution for SB/SFU Full area (SBSFU + SE + Keys). Inner region inside the Region 0
  */
...
#define SFU_PROTECT_MPU_AREA_SFUEN_SREG_0       0xFFU                                   /*!< Subregion mask to deactivate region 1 unexpected MPU subregions: compute at run time */
#define SFU_PROTECT_MPU_AREA_SFUEN_SREG_1       0xFFU                                   /*!< Subregion mask to deactivate region 2 unexpected MPU subregions: compute at run time*/

/**
  * @brief Region 3 - Vector Table: Vector Table must be Read-Only under privileged access. Inner region inside the Region 1
  */
#define SFU_PROTECT_MPU_AREA_VECT_START         ((uint32_t) INTVECT_START)
...
#define SFU_PROTECT_MPU_AREA_VECT_SREG          0x00U                                    /*!< Subregion mask to deactivate unexpected MPU subregions */

/**
  * @brief Region 4 - Inner region inside the Region 0 . The Option Bytes. Once the Option Bytes have been set, only reading will be possible
  */
...
#define SFU_PROTECT_MPU_AREA_OB_BANK1_SREG      0x00U                   /*!< Subregion mask to deactivate unexpected MPU subregions:
                                                                         * 0x1F should be set but it would have no effect as we have a 64B region */


/**
  * @brief Region 5 - SRAM1 Area. Execution is already disabled thanks to the region 0.
  *                   So Region 5 is useless in this case.
  *                   This region is kept only for having the same number of regions with and without MPU isolation.
  */
#define SFU_PROTECT_MPU_AREA_SRAM_START         0x00U /*!< not used */
#define SFU_PROTECT_MPU_AREA_SRAM_SIZE          0x00U /*!< not used */
#define SFU_PROTECT_MPU_AREA_SRAM_PERM          0x00U /*!< not used */
#define SFU_PROTECT_MPU_AREA_SRAM_EXEC          0x00U /*!< not used */
#define SFU_PROTECT_MPU_AREA_SRAM_SREG          0x00U /*!< not used */


/**
  * @brief  Region 6 - Peripherals Area
  */
#define SFU_PROTECT_MPU_AREA_PERIPH_START       PERIPH_BASE
#define SFU_PROTECT_MPU_AREA_PERIPH_SIZE        MPU_REGION_SIZE_512MB                 /*!< To be coherent with the peripherals memory area */
...
#define SFU_PROTECT_MPU_AREA_PERIPH_SREG        0x00U                                 /*!< Subregion mask to deactivate unexpected MPU subregions */
