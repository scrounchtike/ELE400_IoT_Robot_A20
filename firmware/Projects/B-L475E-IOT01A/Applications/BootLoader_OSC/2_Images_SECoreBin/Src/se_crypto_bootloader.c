...
#include "se_key.h"               /* required to access the keys when not provided as input parameter (metadata authentication) */
...
/** @defgroup  SE_CRYPTO_BOOTLOADER SE Crypto for Bootloader
  ...
  *                        \li the ECC DSA without encryption crypto scheme is supported (@ref SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256),
  *                        \li the ECC DSA+AES-CBC crypto scheme is supported (@ref SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256).
  * @{
  */
 ...
 *  @note   We do not use local variable(s) because we want this to be in the protected area (and the stack is not protected),
...
#define SE_CLEAN_UP_FW_KEY() \
    do { \
      (void)memset(m_aSE_FirmwareKey, 0x00, SE_SYMKEY_LEN); \
  } while(0)
...
#define SE_CLEAN_UP_PUB_KEY() \
    do { \
      (void)memset(m_aSE_PubKey, 0x00, SE_ASYM_PUBKEY_LEN); \
  } while(0)
 ...
 *  @note  These functions are not part of the common crypto code because they are used by the bootloader services only.
  * @{
  */
...
/* no high level wrappers provided for AES primitives: the Cryptolib APIs are called directly from  the services (@ref SE_CRYPTO_BOOTLOADER_Exported_Functions) */
                                                   ...
                                                   uint8_t *MessageDigest, int32_t *MessageDigestLength)
{
  ...
  if (0 == ret)
  {
    ret = mbedtls_sha256_update_ret(&ctx, InputMessage, (size_t)InputMessageLength);   /* casting is fine because size_t is unsigned int in ARM C */
...
#error "The current example does not support the selected crypto scheme."
  ...
  ret = mbedtls_cipher_setup(&(m_AESGCMctx.mbedAesGcmCtx), mbedtls_cipher_info_from_values(MBEDTLS_CIPHER_ID_AES, SE_SYMKEY_LEN * 8, MBEDTLS_MODE_GCM));
...
/**
  ...
  * @retval SE_ErrorStatus SE_SUCCESS if successful, SE_ERROR otherwise.
  */
SE_ErrorStatus SE_CRYPTO_Encrypt_Append(const uint8_t *pInputBuffer, int32_t InputSize, uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  SE_ErrorStatus e_ret_status = SE_ERROR;
#if (SECBOOT_CRYPTO_SCHEME == SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM)
  int ret; /* mbedTLS return code */
  ...
  /* The cast "(size_t *)pOutputSize" is valid because we should not get more bytes than InputSize so this fits in an int32_t */
  ret = mbedtls_cipher_update(&(m_AESGCMctx.mbedAesGcmCtx), pInputBuffer, (size_t)InputSize, pOutputBuffer, (size_t *)pOutputSize);
...
#error "The current example does not support the selected crypto scheme."
  ...
  if (0 == ret)
  {
    /* save the GCM tag too : this is the firmware tag (we will authenticate the firmware at the end of the decrypt operation) */
    memcpy(m_AESGCMctx.aTag, (uint8_t *)pxSE_Metadata->FwTag, SE_TAG_LEN);
  ...
  ret = mbedtls_cipher_setup(&(m_AESCBCctx.mbedAesCbcCtx), mbedtls_cipher_info_from_values(MBEDTLS_CIPHER_ID_AES, SE_SYMKEY_LEN * 8, MBEDTLS_MODE_CBC));
...
/**
  ...
  * @retval SE_ErrorStatus SE_SUCCESS if successful, SE_ERROR otherwise.
  */
SE_ErrorStatus SE_CRYPTO_Decrypt_Append(const uint8_t *pInputBuffer, int32_t InputSize, uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  SE_ErrorStatus e_ret_status = SE_ERROR;
#if ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM) )
  int ret; /* mbedTLS return code */
...
#if (SECBOOT_CRYPTO_SCHEME == SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM)
  /* mbedTLS : casting to (size_t *) is fine here because the length cannot be more than InputSize so no risk of overflow */
  ret = mbedtls_cipher_update(&(m_AESGCMctx.mbedAesGcmCtx), pInputBuffer, (size_t)InputSize, pOutputBuffer, (size_t *)pOutputSize);
...
#elif (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256)
  /* standard mbedTLS call for AES CBC decrypt : the cast to (size_t *) is valid because the size cannot be more than InputSize so no risk of overflow
   * The buffer is provided to mbedTLS without checking the AES block alignment: it is up to the caller to make sure the InputSize is a multiple of the AES block (16 bytes).
   */
  ret = mbedtls_cipher_update(&(m_AESCBCctx.mbedAesCbcCtx), pInputBuffer, (size_t)InputSize, pOutputBuffer, (size_t *)pOutputSize);
...
/**
  ...
  *        It is a wrapper of the mbedTLS crypto or hash function included in the protected area used to initialize the FW authentication procedure.
...
SE_ErrorStatus SE_CRYPTO_AuthenticateFW_Init(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
   ...
   */
#if (SECBOOT_CRYPTO_SCHEME == SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM)
  e_ret_status = SE_CRYPTO_Encrypt_Init(pxSE_Metadata);
#elif ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) )
  int ret; /* mbedTLS return code */
...
/**
  ...
  *        It is a wrapper of the mbedTLS crypto or hash function included in the protected area used during the FW authentication procedure.
  * @param pInputBuffer: pointer to Input Buffer.
  * @param InputSize: Input Size (bytes).
  * @param pOutputBuffer: pointer to Output Buffer.
  * @param pOutputSize: pointer to Output Size (bytes).
  * @retval SE_ErrorStatus SE_SUCCESS if successful, SE_ERROR otherwise.
  */
SE_ErrorStatus SE_CRYPTO_AuthenticateFW_Append(const uint8_t *pInputBuffer, int32_t InputSize, uint8_t *pOutputBuffer, int32_t *pOutputSize)
{
  SE_ErrorStatus e_ret_status = SE_ERROR;
  /*
   * Depending on the crypto scheme, the Firmware Tag (signature) can be:
...
#elif ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) )
  int ret = mbedtls_sha256_finish_ret(&m_SHA256ctx, pOutputBuffer);
...
SE_ErrorStatus SE_CRYPTO_Authenticate_Metadata(SE_FwRawHeaderTypeDef *pxSE_Metadata)
{
...
#if ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CBC_SHA256) || (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) )
  /* Variable used to store the Asymmetric Key inside the protected area (even if this is a public key it is stored like the secret key).
  ...
  /* the key to be used for crypto operations (as this is a pointer to m_aSE_FirmwareKey or m_aSE_PubKey it can be a local variable, the pointed data is protected) */
  uint8_t *pKey;
  ...
  ret = mbedtls_cipher_update_ad(&(m_AESGCMctx.mbedAesGcmCtx), (uint8_t *)pxSE_Metadata, (((int32_t)SE_FW_HEADER_TOT_LEN) - SE_TAG_LEN));

  if (0 == ret)
  {
    /* We expect 0 byte to be flushed in the buffer "fw_raw_header_output" so the cats to (size_t *) is valid */
    ret = mbedtls_cipher_finish(&(m_AESGCMctx.mbedAesGcmCtx), fw_raw_header_output, (size_t *)&fw_raw_header_output_length);
  }
