...
sbsfuelf=$projectdir"/../2_Images_SBSFU/SW4STM32/B-L475E-IOT01_2_Images_SBSFU/Debug/SBSFU.elf"
...
ret=$?
if [ $ret == 0 ]; then 
  command=$cmd" "$prepareimage" pack  -k "$ecckey" -p 1 -r 20 -v "$version" -f "$bin" -t "$sign" "$sfb" -o 512" 
  $command >> $projectdir"/output.txt"
  ret=$?
  if [ $ret == 0 ]; then
    command=$cmd" "$prepareimage" header -k  "$ecckey"  -p 1 -r 20 -v "$version"  -f "$bin" -t "$sign" -o 512 "$headerbin
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
