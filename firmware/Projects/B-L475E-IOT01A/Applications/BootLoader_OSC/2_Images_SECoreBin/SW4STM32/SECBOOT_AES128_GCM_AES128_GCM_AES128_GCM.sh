...
sbsfuelf="$SecureDir/2_Images_SBSFU/SW4STM32/B-L475E-IOT01_2_Images_SBSFU/Debug/SBSFU.elf"
current_directory=`pwd`
...
if [ $ret == 0 ]; then
  ...
  ret=$?
  if [ $ret == 0 ]; then 
    command=$cmd" "$prepareimage" pack -k "$oemkey"  -r 72 -v "$version" -n "$nonce" -f "$sfu" -t "$sign" "$sfb" -o 512"
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
      command=$cmd" "$prepareimage" header -k  "$oemkey" -r 72 -v "$version"  -n "$nonce" -f "$sfu" -t "$sign" -o 512 "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret == 0 ]; then
        ...
        if [ $ret == 0 ] && [ $# == 5 ]; then
          ...
          if [ $? == 0 ]; then
            ...
            # On Linux: make sure that STM32_Programmer_CLI is in the PATH before calling this script
