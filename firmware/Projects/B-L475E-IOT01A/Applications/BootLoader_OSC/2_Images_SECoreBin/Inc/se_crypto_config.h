...
/** @defgroup SE_CRYPTO_CONFIG SE Crypto scheme configuration
  ...
  *        These settings apply to the services used by the bootloader only, not to the services called by the User Application.
  * @{
  */
 ...
 *                           !WARNING: SFU_IMAGE_PROGRAMMING_TYPE must be configured to SFU_CLEAR_IMAGE in SBSFU.
 *                           Authenticate Firmware Image using SHA256 on clear Firmware (SHA256 stored in the authenticated metadata).
 *
 ...
 *                           Authenticate Firmware image using AES128-GCM tag.
 */
