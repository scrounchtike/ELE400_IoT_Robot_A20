...
set "command=%python%%prepareimage% pack -k %ecckey% -p 1 -r 20 -v %version% -f %bin% -t %sign% %sfb% -o 512 >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% header -k %ecckey% -p 1 -r 20 -v %version% -f %bin% -t %sign%  -o 512 %headerbin% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
