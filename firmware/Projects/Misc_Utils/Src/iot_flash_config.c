...
#if defined(STM32F769xx) && defined(BOOTLOADER)
/* Workaround bug CubeProgrammer */
#include "MDK-ARM/mapping_sbsfu.h"
const  user_config_t    *lUserConfigPtr = (void*) (SB_REGION_ROM_END + 1);
#else
...
int updateTLSCredentials(void)
{
  ...
  if ( (ret == 0) && app_needs_device_keypair() )
  {
    ...
    {
#ifndef SKIP_DEVICE_CERTIFICATE
...
/**
  * @brief  Get the MQTT broker address ( endpoint) from Flash memory.
...
/**
  ...
  *         Download a FW file from an HTTP URL and program it to FLASH memory.
  */
#define DEFAULT_FW_URL      "http://192.168.1.1/Project.sfb"
#define MAX_FW_URL_LENGTH   128
...
int updateFirmwareVersion()
{
  ...
  if (Button_WaitForPush(5000))
  {
    ...
    printf("\nEnter the URL of the new firmware file (Ex: %s): ", fw_url);
    getInputString(fw_url, sizeof(fw_url));
    msg_info("read: --->\n%s\n<---\n", fw_url);

    printf("Downloading and programming the new firmware.\n");

    int ret = rfu_update(fw_url, lUserConfigPtr->tls_root_ca_cert);
    switch (ret)
    {
      case RFU_OK:
        printf("\nProgramming done.\n");
        /* we have to reboot for the new firmware to be installed */
        printf("Reboot\n");
        HAL_Delay(1000U);
        NVIC_SystemReset();
        break;
...
/**
  * @brief  Update the iot_state in FLASH memory.
  * @param  In: iot_state_t *   info to be stored
  * @retval 0:  Success
  *        -1:  Failure
  */
int setIoTState(iot_state_t *state)
{
  ...
  if (FLASH_update((uint32_t)&lUserConfigPtr->iot_state, state, sizeof(iot_state_t)) < 0)
  {
    msg_error("Failed programming the IOT state into Flash.\n");
    ret = -1;
  }
...
/**
  * @brief  retrieve the iot_state from FLASH memory.
  * @param  In: iot_state_t **   structure to be filled
  * @retval 0:  Success
  *        -1:  Failure
  */
int getIoTState(const iot_state_t **state)
{
  ...
  if (state != NULL)
  {
    if (lUserConfigPtr->iot_state.magic == USER_CONF_MAGIC)
    {
      *state = &lUserConfigPtr->iot_state;
      ret = 0;
    }
    else
    {
